...
bool Map::validate() const {
    ...
    bool result = areContinentsConnected() && isConnectedGraph();
...
Map* MapLoader::loadMap(const string& filename) {
    ...
    // Variables to track the presence of the required sections
    bool hasMapSection = false;
    bool hasContinentsSection = false;
    bool hasTerritoriesSection = false;

    string line;

    // First pass: Check for the required sections
    while (getline(file, line)) {
        if (line == "[Map]") {
            hasMapSection = true;
        } else if (line == "[Continents]") {
            hasContinentsSection = true;
        } else if (line == "[Territories]") {
            hasTerritoriesSection = true;
        }
    }

    // If any of the sections are missing, return nullptr (invalid map file)
    if (!hasMapSection || !hasContinentsSection || !hasTerritoriesSection) {
        cerr << "Invalid map file: Missing required section(s) in " << filename << endl;
        file.close();
        return nullptr;
    }

    // Reset file pointer to the beginning to start actual parsing
    file.clear();
    file.seekg(0, ios::beg);

    // Now proceed with the map creation
    Map* map = new Map();
