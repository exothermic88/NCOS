...
bool Continent::isConnectedSubgraph() const {
    ...
    std::unordered_map<std::string, int> territoryCount;
    for (const Territory* territory : territories) {
        territoryCount[territory->getName()]++;
    }

    // Log duplicate territories if any
    for (const auto& pair : territoryCount) {
        if (pair.second > 1) {
            std::cout << "Warning: Territory '" << pair.first << "' is assigned to multiple continents." << std::endl;
        }
    }

    // Perform BFS for the subgraph check
