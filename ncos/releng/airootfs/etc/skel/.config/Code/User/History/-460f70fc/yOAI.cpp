...
bool Map::isTerritoryInOneContinent() const {
    ...
    std::cout << "Territories address: " << territories << std::endl; // Check the address

    std::unordered_map<std::string, std::string> territoryToContinent;

    if (!territories || territories->empty()) {
        std::cout << "Error: Territories vector is empty or null." << std::endl;
        return true; // Or handle accordingly
    }

    // Loop through each territory in the vector
    for (const Territory* territory : *territories) {
        std::cout << "Territory pointer address: " << territory << std::endl; // Check the address
