    ...
    while (!toVisit.empty()) {
        ...
        for (Territory* neighbor : current->getAdjacentTerritories()) {
            ...
            if (isInContinent) {
                ...
                cout << "Neighbor not found in continent's territories: " << neighbor << endl;
            }
        }
    }
...
bool Map::isConnectedGraph() const {
    ...
    if (territories->empty()) {
        cout << "No territories found" << endl;
        return false;
    }
    ...
    toVisit.push((*territories)[0]);
    visited.insert((*territories)[0]);
    cout << "Starting from territory: " << (*territories)[0]->getName() << endl;

    while (!toVisit.empty()) {
        Territory* current = toVisit.front();
        toVisit.pop();
        cout << "Visiting territory: " << current->getName() << endl;

        for (Territory* neighbor : current->getAdjacentTerritories()) {
            if (visited.find(neighbor) == visited.end()) {
                visited.insert(neighbor);
                toVisit.push(neighbor);
                cout << "Found unvisited adjacent territory: " << neighbor->getName() << endl;
            }
        }
    }
...
bool Map::isTerritoryInOneContinent() const {
    unordered_map<string, set<Continent*>> territoryToContinents; // Using a set to allow multiple continents

    for (Continent* continent : *continents) {
        const auto& territories = continent->getTerritories();
        
        for (Territory* territory : territories) {
            cout << "Checking territory: " << territory->getName() << endl; // Debugging line
            
            territoryToContinents[territory->getName()].insert(continent);
            cout << "Territory: " << territory->getName() << " added to continent: " << continent->getName() << endl;
        }
    }
    ...
    for (const auto& entry : territoryToContinents) {
        if (entry.second.size() > 1) {
            cout << "Territory " << entry.first << " is connected to multiple continents." << endl;
            return false; // Found a territory connected to multiple continents
        }
    }

    cout << "All territories belong to one continent." << endl;
    return true; 
}
