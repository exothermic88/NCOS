# gnome-disks
# Autogenerated from man page /usr/share/man/man1/gnome-disks.1.gz
complete -c gnome-disks -l block-device -d 'Switches to the Disks application and selects the block device given by DEVIC…'
complete -c gnome-disks -l format-device -l xid -d 'Shows the "Format Volume" dialog for the block device given by DEVICE (for ex…'
complete -c gnome-disks -l restore-disk-image -d 'Shows the "Restore Disk Image" dialog for the file given by FILE (for example…'
complete -c gnome-disks -s h -l help -d 'Prints a short help text and exits'

